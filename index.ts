...
// cars$.pipe(skip(3),take(5)).subscribe(val => console.log(val))
// #3 ---
...
// cars$
//   .pipe(pluck('make'), debounceTime(500))
//   .subscribe((val) => console.log(val));
...
// merge(cars$.pipe(delay(1000)), trucks$).subscribe((val) => console.log(val));
// #8 ---
...
cars$.pipe(
  filter(car => car.color === 'blue'),
  tap(car => console.log('intermediate', car)),
  delay(500)
).subscribe(car => console.log(car));
*/
// #4 ---
...
// step 1: combine all cars with all trucks (don't use the 'merge' operator)
/*
combineLatest(cars$, trucks$).subscribe(([car, truck]) => console.log('car:', car, 'truck:', truck));
*/

// #6 ---
// step 1: merge all cars with all trucks (don't use the 'combineLatest' operator)
/*
merge(cars$, trucks$).subscribe(vehicle => console.log(vehicle))
*/
// #7 ---
// step 1: merge all cars with all trucks (don't use the 'combineLatest' operator)
// step 2: make sure that the trucks output before the cars
...
// #8 ---
// step 1: only get the cars by make Ford and Volvo
// step 2: merge the trucks into the cars stream and make sure only the trucks of the same brand as the cars will be in the stream return (HINT: take a look at mergeMap and switchMap, only the trucks should be returned)
/*
cars$.pipe(
  filter(car => car.make === 'Ford' || car.make === 'Volvo'),
  mergeMap(car => trucks$.pipe(filter(truck => truck.make === car.make)))
...
// cars$ = [1,2,3,4,5]; trucks$ = ['a', 'b', 'c']; result = [ [1,'a'], [2,'b'], [3,'c'] ]
/*
zip(cars$, trucks$).subscribe(vehicle => console.log(vehicle));
*/
// #11 ---
// step 1: log something every 2000ms (the value can be a static value)
/*
interval(1000).subscribe(() => console.log('a value'));
*/
